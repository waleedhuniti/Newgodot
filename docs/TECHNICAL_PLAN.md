...
- **Client**: not yet decided. Options: a browser/WebGL client (Three.js + Colyseus's JS client SDK — pairs naturally with this stack and is buildable/testable in the same kind of environment as the server) or a native engine client later once someone can actually run/test an editor. Not blocking further backend work.
...
None of this required Unity or any GUI tool — it was written, run, and tested directly via `curl` and small Node test scripts in this session.
...
1. Wire Redis into the realtime server for session/presence tracking (currently each Shard room only knows about its own connected clients).
2. Persist creature position/roster changes from the realtime server back through the backend API (right now movement is synced live but not saved — a disconnect loses position, which is fine for MVP but should be flagged).
3. Add a `capture_attempt` message handler to `ShardRoom`, backing quest MQ05/the capture mechanic (`docs/GAME_DESIGN.md` §4.2) with real wild-spawn state.
4. Decide on and start the client (Three.js/web is the leading candidate — see §1).
5. Docker Compose for local dev (Postgres + Redis + both Node services) so a new team member doesn't have to manually install/start services like this session did.
...
/client        Not started — see §1 for options
```
